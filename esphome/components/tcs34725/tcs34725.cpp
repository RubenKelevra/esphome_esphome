...
static const float RED_CHANNEL_COUNTS_TO_IRRADIANCE = 0.9430243216495414f;
static const float GREEN_CHANNEL_COUNTS_TO_IRRADIANCE = 0.7023339951685692f;
static const float BLUE_CHANNEL_COUNTS_TO_IRRADIANCE = 0.621110158277791f;
...
void TCS34725Component::calculate_temperature_and_lux_(uint16_t r, uint16_t g, uint16_t b, float current_saturation,
  ...
  if (current_saturation >= sat_limit) {
    if (this->integration_time_auto_) {
      ...
      ESP_LOGW(TAG,
               "Saturation too high, sample with saturation %.1f above limit (%.1f). Lux/color temperature cannot be reliably calculated, reduce integration/gain or use a grey filter.", current_saturation, sat_limit);
      return;
    }
  }
 ...
 *          Lowest raw value reported by the sensor
 */
void TCS34725Component::calculate_irradiance_(uint16_t r, uint16_t g, uint16_t b, float current_saturation, uint16_t min_raw_value) {
  ...
  if (min_raw_value <= min_raw_limit) {
    ESP_LOGW(TAG,
             "Saturation too low, sample with saturation %d (raw value) below limit (%d). Irradiance cannot be reliably calculated.", min_raw_value, min_raw_limit);
    return;
  }
  ...
  if (current_saturation >= sat_limit) {
    if (this->integration_time_auto_) {
      ...
      ESP_LOGW(TAG,
               "Saturation too high, sample with saturation %.1f above limit (%.1f). Irradiance cannot be reliably calculated, reduce integration/gain or use a grey filter.", current_saturation, sat_limit);
      return;
    }
  }
  ...
  ESP_LOGD(TAG, "Calculated irradiance - R: %.2f µW/cm2, G: %.2f µW/cm2, B: %.2f µW/cm2", this->irradiance_r_, this->irradiance_g_, this->irradiance_b_);
}
...
void TCS34725Component::update() {
  ...
  ESP_LOGD(TAG,
           "Calculated: Red Irad=%.2f µW/cm²,Green Irad=%.2f µW/cm², Blue Irad=%.2f µW/cm², Sensor Sat=%.2f%%, Illum=%.1f lx, Color Temp=%.1f K",
           this->irradiance_r_, this->irradiance_g_, this->irradiance_b_, current_saturation, this->illuminance_, this->color_temperature_);
